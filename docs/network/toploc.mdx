...
<Note>
  **Status: Planned v1** — not live in production routing or settlement. Signed receipts (v0.4 settlement receipts and v0.3 legacy receipts) are the live verification primitive today.
</Note>
...
| Question | Signed receipt (live) | TOPLOC (planned v1) |
| --- | --- | --- |
| Did the provider sign this `(prompt, output)`? | Yes, if the receipt verifies | Complementary |
...
- Verifier runs on weights aligned with the catalog `model_hash`
...
| Weight file drift | A reported-hash mismatch against the catalog already fails routing and settlement |
...
TOPLOC is **not** a Malibu invention. Malibu plans to integrate it as the v1 buyer-response attestation layer. No TOPLOC code runs in the provider or coordinator today.

## Related

- [Threat model](/network/threat-model) — model-swap adversary A3
- [Receipts](/network/receipts) — live receipt primitive
